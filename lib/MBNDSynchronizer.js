...
class MBNDSynchronizer {
  ...
  initiate(action) {
    ...
    if (Object.keys(options).length) {
      console.log(
        `MBNDS v${packageJson.version} running with following options:`,
        options,
      );
    }
    ...
    if (action === "fullSync") {
      paths.csvFilePath =
        options.csv ??
        path.join(paths.defaultWorkingDirectory, paths.defaultCsvFileName);
    ...
    paths.dbFilePath =
      options.db ??
      path.join(paths.defaultWorkingDirectory, paths.defaultDbFileName);
    ...
    if (
      ...
      !dayjs(
        dayjs().format(options.datetimeFormat),
        options.datetimeFormat,
      ).isValid()
    ) {
      throw new Error(
  ...
  getUser() {
    ...
    const user = options.user
      ? database
          .prepare("SELECT * FROM user WHERE user_name = ?")
          .get(options.user)
  ...
  async run(action) {
    ...
    try {
      ...
      this.database.close();
      console.log(
        `${action} completed successfully ${dayjs.duration(dayjs().diff(this.start)).humanize(true)}`,
      );
    } catch (e) {
  ...
  async processCsv(mode, onTrack = null) {
    ...
    const colParser = {
      ...
      lastPlayed: (item) =>
        dayjs(item, syncOptions.datetimeFormat).isValid()
          ? dayjs(item, syncOptions.datetimeFormat).utc()
          : null,
  ...
  async fullSync() {
    ...
    console.log(
      `${paths.csvFilePath} parsed successfully, ${totalEligibleTracks} potential tracks to be updated`,
    );
        ...
        `,
          ...
          if (!foundTrack) {
            ...
            if (options.verbose || options.showNotFound) {
              console.error(
                `track not found. path: ${track.filePath} | filename: ${track.filename}`,
              );
            }
            return;
          }
          ...
          const hasExistingAnnotation =
            foundTrack.annotation_play_count !== null ||
            foundTrack.annotation_rating !== null;
          ...
          const update = {};
          if (
            track.rating !== annotation.rating &&
            (options.forceRatings || track.rating > annotation.rating)
          ) {
          ...
          if (track.playCount !== annotation.play_count) {
            ...
            if (
              options.first &&
              annotation.play_count + track.playCount > annotation.play_count
            ) {
  ...
  async albumsSync(albumsToUpdate = null) {
    ...
    const albumsData = this.getAlbumsWithStats(
      user,
      albumsToUpdate ? [...albumsToUpdate] : null,
    );
    ...
    await this.database.executeTransaction(() => {
      for (const albumData of albumsData) {
        ...
        const needsCreate =
          albumData.album_play_count === null &&
          albumData.album_rating === null;
        ...
        if (albumData.tracks_rated_count > albumData.total_tracks * 0.5) {
          const newRating = Math.round(
            albumData.tracks_rating_sum / albumData.tracks_rated_count,
          );
          if (
            newRating !== currentRating &&
            (options.forceRatings || newRating > currentRating)
          ) {
    ...
    if (this.options.verbose) {
      console.log(
        `Using ${hasMediaFileArtists ? "new" : "old"} artist schema (${
          hasMediaFileArtists
            ? "media_file_artist junction table"
            : "direct artist_id"
        })`,
      );
    }
  ...
  async artistsSync(artistsToUpdate = null) {
    ...
    const artistsData = this.getArtistsWithStats(
      user,
      artistsToUpdate ? [...artistsToUpdate] : null,
    );
    ...
    await this.database.executeTransaction(() => {
      for (const artistData of artistsData) {
        ...
        const needsCreate =
          artistData.artist_play_count === null &&
          artistData.artist_rating === null;
        ...
        if (
          artistData.total_tracks > 1 &&
          artistData.tracks_rated_count > artistData.total_tracks * 0.5
        ) {
          const newRating = Math.round(
            artistData.tracks_rating_sum / artistData.tracks_rated_count,
          );
          if (
            newRating !== currentRating &&
            (options.forceRatings || newRating > currentRating)
