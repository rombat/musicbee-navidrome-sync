        ...
        await Promise.all(
          trackBatch.map(track =>
            limit(async () => {
              ...
              const update = {};
              if (track.rating !== annotation.rating && (this.options.forceRatings || track.rating > annotation.rating)) {
              ...
              if (track.playCount !== annotation.play_count) {
                ...
                if (this.options.first && annotation.play_count + track.playCount > annotation.play_count) {
      ...
      albumsData.map(albumData =>
        limit(async () => {
          ...
          if (albumData.tracks_rated_count > albumData.total_tracks * 0.5) {
            ...
            if (newRating !== currentRating && (this.options.forceRatings || newRating > currentRating)) {
