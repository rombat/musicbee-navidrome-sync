...
let backupFilePath;
let dbFilePath = './data/navidrome.db';
let csvFilePath = './data/MusicBee_Export.csv';

/**
 * TODO: define a working dir (currently, data)
 * store all pathes in an object (backupFilePath, etc)
 * use path options
 */
...
exports.fullSync = async options => {
  ...
  if (!options.csv && !options.db && fs.existsSync('./navidrome.db') && fs.existsSync('./MusicBee_Export.csv')) {
    for (const file of ['navidrome.db', 'MusicBee_Export.csv']) {
      fs.renameSync(`./${file}`, `./wtf/${file}`);
    }
  }
  ...
  try {
    ...
    console.log(`data/MusicBee_Export.csv parsed successfully, ${musicBeeCollection.length} potential tracks to be updated`);
    ...
    let tracksAnnotations = await Promise.all(
      musicBeeCollection.map((track, index) =>
        limit(async () => {
    ...
    console.log(`Full sync completed successfully ${dayjs.duration(dayjs().diff(start)).humanize(true)}`);
  } catch (e) {
    handleGlobalError(e);
  }
};
...
exports.albumsSync = async options => {
  ...
  try {
    ...
    await sequelize.close();
    console.log(`Albums sync completed successfully ${dayjs.duration(dayjs().diff(start)).humanize(true)}`);
  } catch (e) {
    handleGlobalError(e);
  }
};
...
exports.artistsSync = async options => {
  ...
  try {
    ...
    await sequelize.close();
    console.log(`Artists sync completed successfully ${dayjs.duration(dayjs().diff(start)).humanize(true)}`);
  } catch (e) {
    handleGlobalError(e);
  }
};
...
const backupDbFile = () => {
  ...
  console.log(`DB has been backed up to data/navidrome_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}_backup.db`);
};
const restoreDbFile = () => {
  fs.copyFileSync(backupFilePath, './data/navidrome.db');
  for (const file of ['navidrome.db-shm', 'navidrome.db-wal']) {
...
process.on('SIGINT', restoreDbFile);
process.on('SIGTERM', restoreDbFile);
