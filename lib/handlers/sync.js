...
exports.fullSync = async options => {
  ...
  fs.copyFileSync('./data/navidrome.db', `./data/navidrome_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}_backup.db`);
  ...
  let musicBeeCollection = await csv2json({
    ...
    colParser: {
      ...
      dateAdded: item => dayjs(item, 'DD/MM/YYYY HH:mm'),
      lastPlayed: item => (dayjs(item, 'DD/MM/YYYY HH:mm').isValid() ? dayjs(item, 'DD/MM/YYYY HH:mm') : null),
      dateModified: item => dayjs(item, 'DD/MM/YYYY HH:mm'),
  ...
  const { Track, Annotation, User, Album, Artist } = sequelize.models;
