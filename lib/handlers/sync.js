...
exports.fullSync = async options => {
  ...
  try {
    ...
    const tracksAnnotations = await Promise.all(
      musicBeeCollection.map(track =>
        limit(async () => {
          ...
          if (track.love > annotation.starred) {
            ...
            update.starred_at = track.lastPlayed || new Date(); // this data is not available with MB
          }
    ...
    console.log(`${tracksAnnotations.filter(Boolean).length} tracks updated`);
    ...
    console.log(`Sync completed successfully ${dayjs.duration(dayjs().diff(now)).humanize(true)}`);
 ...
 * @param {Object}  user
 * @param {Set<string>} albumsToUpdate
 * @param {Object}  limit
...
const updateAlbums = async (sequelize, user, albumsToUpdate, limit, options) => {
  ...
  if (albumsToUpdate.size) {
    ...
    console.log(`${albumsAnnotations.filter(Boolean).length} albums updated`);
  }
...
const updateArtists = async (sequelize, user, artistsToUpdate, limit, options) => {
  ...
  if (artistsToUpdate.size) {
    ...
    console.log(`${artistsAnnotations.filter(Boolean).length} artists updated`);
  }
