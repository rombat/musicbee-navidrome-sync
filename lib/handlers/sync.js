...
exports.fullSync = async (options, action = 'fullSync') => {
  const start = dayjs();

  initiate(options, action);
...
exports.albumsSync = async (options, action = 'albumsSync') => {
  const start = dayjs();

  initiate(options, action);

  try {
    const sequelize = await dbManager.init(options, paths);
...
exports.artistsSync = async (options, action = 'artistsSync') => {
  const start = dayjs();
...
const backupDbFile = () => {
  ...
  fs.copyFileSync('./data/navidrome.db', paths.backupFilePath);
  console.log(`DB has been backed up to ${paths.backupFilePath}`);
};
const restoreDbFile = () => {
  fs.copyFileSync(paths.backupFilePath, './data/navidrome.db');
