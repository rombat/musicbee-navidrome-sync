...
const { findBestMatch, isDateAfter } = require('../helpers/helpers');
...
class MBNDSynchronizer {
  ...
  processCsv = async (mode, tracksHandler = null, options = {}) => {
    ...
    const colParser = {
      playCount: 'number',
      rating: item => {
        let rating = Number.parseInt(item);
        if (!rating) return 0;
  ...
  getArtistsWithStats = async (user, artistIds = null) => {
    ...
    if (this.options.verbose) {
      console.log(
        `Using ${hasMediaFileArtists ? 'new' : 'old'} artist schema (${hasMediaFileArtists ? 'media_file_artist junction table' : 'direct artist_id'})`
      );
    }
