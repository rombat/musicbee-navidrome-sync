...
class MBNDSynchronizer {
  ...
  fullSync = async () => {
    ...
    // Include artists if using many-to-many relationship
    ...
    let tracksAnnotations = await Promise.all(
      musicBeeCollection.map(track =>
        limit(async () => {
          ...
          // foundTrack.artist_id is not accurate as tracks can have multiple artists (from 0.55.0)
          if (hasMediaFileArtists) {
            if (foundTrack.tracks && foundTrack.tracks.length > 0) {
              foundTrack.tracks.forEach(artist => {
                artistsToUpdate.add(artist.id);
              });
            }
          } else {
            // Legacy: use direct artist_id foreign key (before 0.55.0)
            if (foundTrack.artist_id) {
